...
## .babelrc文件
`.babelrc`文件存储了react的配置文件,此文件被隐藏，一般是需要命令`npm run eject`才能显示,若是执行的时候报错，则一般是**文件被修改**,解决方案:删掉`.git、.gitignore`,再执行会暴露出`script、config`文件夹g;或者先`git push`，然后在`npm run eject`